...
Implementation of the Deals application inspired by the DynamoDB book.
...
2. How do I generate an event, e.g eventbridge scheduled test event, to simulate a lambda invocation?
...
```bash
ripple-contacts$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ripple-contacts$ sam local start-api
ripple-contacts$ curl http://localhost:3000/
```
...
```bash
ripple-contacts$ sam logs -n HelloWorldFunction --stack-name ripple-contacts --tail
```
...
```bash
ripple-contacts$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name ripple-contacts
```
